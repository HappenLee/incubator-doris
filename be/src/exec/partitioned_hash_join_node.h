...
#ifndef DORIS_EXEC_PARTITIONED_HASH_JOIN_NODE_H
#define DORIS_EXEC_PARTITIONED_HASH_JOIN_NODE_H
...
class PartitionedHashJoinNode : public BlockingJoinNode {
 ...
 private:
  ...
  /// sensitive).
  bool AppendRow(BufferedTupleStream2* stream, TupleRow* row, Status* status);
  ...
  bool AppendRowStreamFull(BufferedTupleStream2* stream, TupleRow* row, Status* status);
  ...
  /// Probes '_current_left_child_row' against the the hash tables and append outputs
  ...
  /// Evaluates some number of rows in '_left_batch' against the probe expressions
  ...
  /// '_current_left_child_row' and 'hash_tbl_iterator_' have been set up to point to the
  ...
  bool inline NextProbeRow(
      PartitionedHashTableCtx* ht_ctx, RowBatch::Iterator* _left_batchiterator,
      int* remaining_capacity, Status* status);

  /// Process probe rows from _left_batch. Returns either if out_batch is full or
  /// _left_batch is entirely consumed.
  ...
  template<int const JoinOp>
  int ProcessProbeBatch(TPrefetchMode::type, RowBatch* out_batch, PartitionedHashTableCtx* ht_ctx,
      Status* status);
  ...
  Status EvaluateNullProbe(BufferedTupleStream2* build);
  ...
  /// consuming the input, sets _left_batch_pos to -1, otherwise, sets it to 0.
  ...
  /// input, sets _left_batch_pos to -1, otherwise, sets it to 0.
  ...
  int MinRequiredBuffers() const {
    ...
    num_reserved_buffers += _join_op == TJoinOp::NULL_AWARE_LEFT_ANTI_JOIN ? 3 : 0;
...
//  std::vector<FilterContext> filters_;
  ...
  /// TODO: fix doris-1948 and remove this.
  ...
  /// The iterator that corresponds to the look up of _current_left_child_row.
  PartitionedHashTable::Iterator hash_tbl_iterator_;
  ...
  /// conjuncts.
  BufferedTupleStream2* null_probe_rows_;
  ...
  class Partition {
   public:
    ...
    BufferedTupleStream2* ALWAYS_INLINE build_rows() { return build_rows_; }
    BufferedTupleStream2* ALWAYS_INLINE probe_rows() { return probe_rows_; }
    PartitionedHashTable* ALWAYS_INLINE hash_tbl() const { return hash_tbl_.get(); }
   ...
   private:
    ...
    /// inserted.
    bool InsertBatch(TPrefetchMode::type prefetch_mode, PartitionedHashTableCtx* ctx,
        RowBatch* batch, const std::vector<BufferedTupleStream2::RowIdx>& indices);
    ...
    boost::scoped_ptr<PartitionedHashTable> hash_tbl_;
    ...
    BufferedTupleStream2* build_rows_;
    BufferedTupleStream2* probe_rows_;
  };
  ...
  typedef int (*ProcessProbeBatchFn)(PartitionedHashJoinNode*,
      TPrefetchMode::type, RowBatch*, PartitionedHashTableCtx*, Status*);
  ...
  typedef bool (*InsertBatchFn)(Partition*, TPrefetchMode::type, PartitionedHashTableCtx*,
      RowBatch*, const std::vector<BufferedTupleStream2::RowIdx>&);
