...
public:
    ...
    const std::vector<SlotDescriptor*>& get_slot_descs() const { return *_slot_descs; }
private:
    class TupleComparator {
    public:
        TupleComparator(const Schema* schema, const std::vector<SlotDescriptor*>& slot_descs);
...
public:
    using Table = SkipList<char*, TupleComparator>;
...
private:
    void _aggregate_two_tuple(const Tuple* tuple, Tuple* tuple_in_skiplist);
    Tuple* _init_copy_tuple_in_load(const Tuple *tuple);
    ...
    const std::vector<SlotDescriptor*>* _slot_descs;
    // so sometime type like:
    //  DECIMAL, DATE, DATETIME
    // The storage engine data mem layout is different from query layer, so need to
    // init before use it
    // another type like:
    //  HLL, BITMAP
    // need to init first before do update and finalize
    std::vector<std::pair<size_t, SlotDescriptor*>> _need_init_slot_descs;
    KeysType _keys_type;

    TupleComparator _row_comparator;
